...
class CBR_Admin_Toolbar {
	...
	*/
	...
	public function set_checkout_country() {
		...
		echo json_encode( array('success' => 'true', 'country' => $country, 'countrycode' => $cookie_country ) );die();
	}
	...
	function cbr_adminbar_rendar(){
		...
		if(is_admin_bar_showing()){
			...
			if ( get_option('wpcbr_debug_mode') != '1' || is_admin() ) return;
			global $fzpcr;
			$user = wp_get_current_user();
			$user_role = array('administrator','shop_manager');
			
			if ( isset($user->roles[0]) && !in_array(  $user->roles[0], $user_role ) ) return;
			...
			<style type="text/css">
				...
				#wpadminbar .display-country-for-customer .ab-icon:before{background-image: url('<?php echo $wsmab_zorem_icon; ?>');background-repeat: no-repeat;background-position: 0 50%;padding-left: 12px !important;background-size: 12px;content: '';top: 2px;vertical-align: middle;}
			</style>
			<?php
		 	$html = ob_get_clean();
			$wp_admin_bar->add_menu(
