...
class CBR_Product_Restriction {
	...
	*/
    function init() {
		...
		if( get_option('product_visibility') == 'hide_completely' || ( get_option('wpcbr_make_non_purchasable') == '1' && get_option('product_visibility') == 'hide_catalog_visibility' ) || get_option('product_visibility') == 'show_catalog_visibility' ){				
	...
	*/
	function vc_product_restricted_query( $query_args ){
		...
		// shop, category, search = hidden, single page visible
		if( get_option('product_visibility') == 'hide_catalog_visibility' && is_product() ){
			return $query_args;
		}
		...
		if(isset($query_args['post__in'])){
			foreach ( $query_args['post__in'] as $product_id ) {
				if( $this->is_restricted_by_id( $product_id ) != '1'){
					$post__in[] = $product_id;
				}
			}
		}
	...
	*/
	function restricted_related_products($related_posts, $product_id, $arg){
		...
		if(get_option('product_visibility') != 'hide_completely') return $related_posts;
	...
	*/
	function restricted_upsell_products( $upsell_ids,  $instance ){
		...
		if(get_option('product_visibility') != 'hide_completely') return $upsell_ids;
